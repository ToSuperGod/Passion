...
def train(model):
    ...
    with fluid.dygraph.guard():
        ...
        for epoch in range(epoch_num): # 5轮训练
            for batch_id,data in enumerate(train_loader()): # 一个data10张照片，共400张照片，40个batch

                x_data,y_data=data
                img = fluid.dygraph.to_variable(x_data)  # 数据格式的转换
                # print("类型转换后img type:",type(img))
...
class LeNet(fluid.dygraph.Layer):
    def __init__(self,num_classes=1):
        ...
        # 输入图片的通道数、滤波器个数、滤波器大小、步长默认为1、
        self.conv1 = Conv2D(num_channels=3,num_filters=6,filter_size=5,act='sigmoid')
        # 池化层特征融合，降维
        # 大小、步长
        self.pool1 = Pool2D(pool_size=2,pool_stride=2,pool_type='max')# 输出神经元个数110*110*6
        self.conv2 = Conv2D(num_channels=6,num_filters=16,filter_size=5,act='sigmoid')
        self.pool2 = Pool2D(pool_size=2,pool_stride=2,pool_type='max')# 输出神经元个数53*53*16
        # 创建第三个卷积层
        self.conv3 = Conv2D(num_channels=16,num_filters=120,filter_size=4,act='sigmoid') # 输出神经元个数50*50*120
...
if __name__ == '__main__':
    ...
    # 根据paddle编译的方式来选择运行的设备，如果编译的cpu版本，则在cpu上运行，如果是编译的gpu版本，则在gpu上运行
