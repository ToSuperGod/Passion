...
class vgg_block(fluid.dygraph.Layer):
    ...
    # 每次调用时执行的计算。应该被所有子类覆盖
    def forward(self, x):
        for item in self.conv_list:
            x = item(x)
        return self.pool(x)

# 3*3的卷积核，图片减小，输出通道数增加
class VGG(fluid.dygraph.Layer):
    def __init__(self, conv_arch=((2, 64),
        ...
        for (num_convs, num_channels) in conv_arch:
            ...
            block = self.add_sublayer('block_' + str(iter_id),  # 添加子层
                                      vgg_block(num_convs, in_channels=in_channels[iter_id],# 输入通道数
                                                out_channels=num_channels))# 输出通道数
